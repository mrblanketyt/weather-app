        ...
        hourly.forEach(hour => {
            ...
            hourlyHTML += `
                <div class="hourly-item">
                    ...
                    <pTo resolve the issues, let's proceed with the changes for `app.js`, ensuring we address autocomplete suggestions, scrolling, and a modern look with boxed UI elements.
    ...
    cityInput.addEventListener('input', function () {
        ...
        if (query.length > 2) {
            fetch(`https://api.weatherapi.com/v1/search.json?key=${apiKey}&q=${query}`)
                .then(response => response.json())
                .then(data => {
                    const suggestions = data.map(city => `<option value="${city.name}">${city.name}</option>`).join('');
